...
bool text_open(char *filename)
{
	...
	if(text_src) free(text_src);

	text_src=(char*)malloc(size);
			...
			for(chn=0;chn<channels;++chn)
			{
				...
				if(chn!=3)//normal channels
				{
					...
					if(note>1)
					{
						...
						if(note<12+2||note>12*6+4) parse_error(off,"Note is out of supported range (C-1..D-6)");
